...
@dataclass(frozen=True)
class BookmakerMarketEdgeArtifact:
    experiment_id: str
    market_policy: str
    ...
    coverage: tuple[BookmakerClaimCoverage, ...]
...
def build_bookmaker_market_edge_artifact(
    *,
    ...
    signal_paths: dict[tuple[Tour, SignalName], Path] = {
    ...
    coverage: list[BookmakerClaimCoverage] = []
    for tour, signal_name in _CLAIMS:
        ...
        if any(row.year > _DEVELOPMENT_END_YEAR for row in rows):
            raise ValueError("MARKET-EDGE-001 bookmaker run is frozen through 2025")
    ...
    family = run_market_edge_family(claim_rows, min_prior_rows=min_prior_rows)
    paths = {
    ...
    return BookmakerMarketEdgeArtifact(
        experiment_id=_EXPERIMENT_ID,
        market_policy=_MARKET_POLICY,
        market_probability_method=_MARKET_PROBABILITY_METHOD,
        qa_required=True,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run frozen MARKET-EDGE-001 on QA-approved bookmaker closes"
    )
...
def main() -> None:
    args = _parse_args()
    artifact = build_bookmaker_market_edge_artifact(
