...
TARGET_PLAYER_NAMES = {"sloanestephens", "janicetjen"}


def _load_forward_module():
    source_path = Path(__file__).with_name("wta_live_prediction_002.py")
    source = source_path.read_text(encoding="utf-8")
    source = source.replace("FULL-STACK-FORWARD-001-", "FULL-STACK-FORWARD-002-")
    module = types.ModuleType("wta_live_prediction_002_forward_002")
...
def _find_target_summary(h) -> dict[str, object]:
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"expected exactly one Stephens-Tjen target, found {len(candidates)}")
    return candidates[0]
...
def main() -> None:
    ...
    if str(status.get("status", "")).lower() not in {"not_started", "scheduled"}:
        raise RuntimeError(f"Stephens-Tjen is no longer pre-match: {status}")
    ...
    receipt = json.loads(Path("provider_batch_anchor_receipt.json").read_text(encoding="utf-8"))
    anchor_comment_id = int(Path("provider_batch_anchor_comment_id.txt").read_text(encoding="utf-8").strip())
    ...
    resolved = {
        "schema_version": "wta-forward-002-target-resolution-v1",
        "forward_protocol": "FULL-STACK-FORWARD-002",
    ...
    Path("forward-002-target-resolution.json").write_text(
