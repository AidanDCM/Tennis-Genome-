...
def apply_web_shadow_settlement_batch(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="web-shadow-settlement-apply-") as temp_name:
        ...
        reported_correct = int(summary.get("correct_prediction_count", -1))
        ...
        for row in rows:
            ...
            if not isinstance(score, str) or not score.strip():
                raise ValueError(
                    "settlement result score must be a non-empty string"
                )
            ...
            if settlement.get("schema_version") != "tennis-genome-web-shadow-v1":
                raise ValueError("unsupported settlement record schema")
            if settlement.get("record_type") != "WEB_SHADOW_SETTLEMENT":
                raise ValueError("unexpected settlement record type")
            ...
            settlement_correct = settlement.get("prediction_correct")
            row_correct = row.get("prediction_correct")
            if not isinstance(settlement_correct, bool) or not isinstance(
                row_correct, bool
            ):
                raise ValueError("settlement correctness flags must be boolean")
            if settlement_correct is not row_correct:
                raise ValueError("settlement correctness differs from batch artifact")

            row_eligible = row.get("evaluation_eligible")
            if not isinstance(row_eligible, bool):
                raise ValueError("settlement evaluation eligibility must be boolean")
            settlement_eligible = _settlement_is_evaluation_eligible(settlement)
            if row_eligible is not settlement_eligible:
        ...
        if computed_eligible != eligible_count:
            raise ValueError("settlement eligible-row denominator drift")
        if settlement_count - computed_eligible != excluded_count:
            raise ValueError("settlement excluded-row denominator drift")
        if reported_correct != computed_correct:
            raise ValueError("settlement correct-count drift")
        if eligible_count == 0:
            if reported_accuracy is not None:
                raise ValueError("empty settlement evaluation cohort published accuracy")
        else:
            try:
                accuracy = float(reported_accuracy)
            except (TypeError, ValueError) as exc:
                raise ValueError("settlement batch accuracy is invalid") from exc
            expected_accuracy = computed_correct / eligible_count
            if not math.isfinite(accuracy) or abs(accuracy - expected_accuracy) > 1e-12:
                raise ValueError("settlement batch accuracy drift")

        batch_receipt_dir = (
            repo_root
            / "web-shadow"
            / "settlement-batches"
            / str(source_workflow_run_id)
        )
        if batch_receipt_dir.exists():
            raise FileExistsError(
                f"settlement batch provenance already exists: {batch_receipt_dir}"
            )
        batch_receipt_created = False
        ...
        try:
            ...
            batch_receipt_dir.mkdir(parents=True)
            batch_receipt_created = True
            shutil.copyfile(
                summary_path,
                batch_receipt_dir / "settlement-batch-summary.json",
            )
            shutil.copyfile(
                manifest_path,
                batch_receipt_dir / "result-batch-manifest.json",
            )
            _write_json(
                batch_receipt_dir / "artifact-receipt.json",
                {
                    "schema_version": (
                        "tennis-genome-web-shadow-settlement-batch-artifact-receipt-v1"
                    ),
                    ...
                    "affected_slate_ids": sorted(
                        {str(row["slate_id"]) for row in rows}
                    ),
            ...
            if batch_receipt_created:
                shutil.rmtree(batch_receipt_dir, ignore_errors=True)
    ...
    return {
        ...
        "settlement_batch_receipt_path": (
            batch_receipt_dir.relative_to(repo_root) / "artifact-receipt.json"
        ).as_posix(),
