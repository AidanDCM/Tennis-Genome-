...
def test_intake_materializes_sorted_model_blind_slate_and_evidence(tmp_path: Path) -> None:
    matches = [
        _match(event_id="LS002", start="2026-09-23T08:00:00+00:00", player_a="Gamma Three", player_b="Delta Four", player_a_id=303, player_b_id=404),
...
def test_intake_filters_doubles_finished_unscheduled_lead_and_horizon(tmp_path: Path) -> None:
    matches = [
        _match(event_id="DOUBLE", start="2026-09-23T06:00:00+00:00", draw_type="D"),
        _match(event_id="DONE", start="2026-09-23T06:00:00+00:00", state="F"),
        _match(event_id="UNSCHED", start="2026-09-23T06:00:00+00:00", unscheduled=True),
        _match(event_id="FOLLOW", start="2026-09-23T06:00:00+00:00", not_before="Followed By"),
...
def test_intake_applies_fixed_capacity_after_deterministic_sort(tmp_path: Path) -> None:
    matches = [
        _match(event_id="LS003", start="2026-09-23T06:00:00+00:00", player_a_id=301, player_b_id=302, player_a="C A", player_b="C B"),
        _match(event_id="LS001", start="2026-09-23T06:00:00+00:00", player_a_id=101, player_b_id=102, player_a="A A", player_b="A B"),
        _match(event_id="LS002", start="2026-09-23T06:00:00+00:00", player_a_id=201, player_b_id=202, player_a="B A", player_b="B B"),
    ]
...
def test_intake_excludes_existing_official_event_id(tmp_path: Path) -> None:
    ...
    matches = [
        _match(event_id="LS001", start="2026-09-23T06:00:00+00:00"),
        _match(event_id="LS002", start="2026-09-23T08:00:00+00:00", player_a_id=303, player_b_id=404, player_a="Gamma Three", player_b="Delta Four"),
    ]
...
def test_intake_rejects_duplicate_upcoming_event_ids(tmp_path: Path) -> None:
    matches = [
        _match(event_id="LS001", start="2026-09-23T06:00:00+00:00"),
        _match(event_id="LS001", start="2026-09-23T08:00:00+00:00", player_a_id=303, player_b_id=404, player_a="Gamma Three", player_b="Delta Four"),
    ]
...
def test_intake_excludes_unsupported_round_id_without_guessing(tmp_path: Path) -> None:
    matches = [
        _match(event_id="QUAL", start="2026-09-23T06:00:00+00:00", round_id=99),
        _match(event_id="MAIN", start="2026-09-23T08:00:00+00:00", round_id=5, player_a_id=303, player_b_id=404, player_a="Gamma Three", player_b="Delta Four"),
    ]
...
def test_intake_refuses_empty_slate_without_mutating_active_slate(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
